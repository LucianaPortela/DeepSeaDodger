...
import sys
import pygame
from pygame import Surface, Rect, KEYDOWN, K_RETURN, K_ESCAPE
from pygame.font import Font
from code.Const import C_WHITE, WIN_WIDTH, WIN_HEIGHT, ENTITY_DAMAGE, C_GREEN, C_RED
...
class Info:
    ...
    def run(self):
        while True:
            ...
            self.info_text(40, "Faça pontos comendo minhocas", C_WHITE, (WIN_WIDTH / 2, WIN_HEIGHT - 295))
            self.info_text(40, "e fuja dos inimigos!", C_WHITE, (WIN_WIDTH / 2, WIN_HEIGHT - 260))
            self.window.blit(self.e1, (WIN_WIDTH * 0.30, 95))
            self.info_text(30, f"Dano: -{ENTITY_DAMAGE['Enemy1']}", C_RED, (WIN_WIDTH * 0.20, 110))
            self.window.blit(self.e2, (WIN_WIDTH * 0.37 - 40, 145))
            self.info_text(30, f"Dano: -{ENTITY_DAMAGE['Enemy2']}", C_RED, (WIN_WIDTH * 0.20, 180))
            self.window.blit(self.e3, (WIN_WIDTH * 0.76, 155))
            self.info_text(30, f"Dano: -{ENTITY_DAMAGE['Enemy3']}", C_RED, (WIN_WIDTH * 0.65, 180))
            self.window.blit(self.e4, (WIN_WIDTH * 0.76, 85))
            self.info_text(30, f"Dano: -{ENTITY_DAMAGE['Enemy4']}", C_RED, (WIN_WIDTH * 0.65, 110))
            self.window.blit(self.e5, (WIN_WIDTH / 2 + 10, 235))
            self.info_text(30, f"Vida: +{-ENTITY_DAMAGE['Enemy5']}", C_GREEN, (WIN_WIDTH / 2 - 50, 245))
            self.info_text(35, "aperte ENTER para iniciar!", C_WHITE, (WIN_WIDTH / 2 - 10, WIN_HEIGHT - 30))
